...
module GlueStore
  class CrawlAll < Base
    ...
    def crawler_and_build
      ...
      @attributes += crawler.data.map { |result| build_attributes(result) }.compact.uniq do |attribute|
    ...
    def remove_old_products
      ...
      remove_products_for_store(store: Product::GLUE_STORE, keep_store_product_ids: store_product_ids)
    end
  end
end
